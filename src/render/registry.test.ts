...
describe("LAYER_ORDER", () => {
  it("defines exactly 8 layers", () => {
    expect(Object.keys(LAYER_ORDER)).toHaveLength(8);
  });
...
describe("validateRegistry", () => {
  ...
  it("reports all 8 layers missing when registry is empty", () => {
    const result = validateRegistry();
    expect(result.valid).toBe(false);
    expect(result.missing).toHaveLength(8);
  });

  it("reports valid when all 8 layers are registered", () => {
